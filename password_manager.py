...
# uuid is for recovery key
import uuid
# for copying recovery key
import pyperclip
# Used for encrypting data
import base64
# Fernet for encrypting data
...
# that key can be recovered in both manners
...
def hashPassword(input):
    # Takes input and turns it into an sha256 hash
    hash1 = hashlib.sha256(input)
    # Turn hash to readable text
    hash1 = hash1.hexdigest()

    return hash1

def firstScreen(hasMasterKey=None):
    # Destroys everything currently present in the screen
    for widget in window.winfo_children():
        widget.destroy()

    window.geometry("250x125")

    # Label for 'Enter Master Password
    ...
    def savePassword():
        if txt.get() == txt1.get():
            ...
            # Create recovery key
            # Creates a random universal unique identifier using uuid4 and then converts it into a hexadecimal number
            ...
            # Check if masterykey exists. If it does, replace it by encrypting it with new password and new recoverykey hash
...
def resetScreen():
    ...
    def checkRecoveryKey():
        ...
        if recoveryKey:
            # unencrypt masterKey and pass it to firstScreen
            cursor.execute("SELECT * FROM masterkey")
            masterKeyEntry = cursor.fetchall()
            if masterKeyEntry:
                masterKeyRecoveryKey = masterKeyEntry[0][2]

                masterKey = decrypt(masterKeyRecoveryKey, base64.urlsafe_b64.encode(kdf().derive(str(txt.get()).encode()))).decode()

                firstScreen(masterKey)
            else:
...
def vaultScreen():
    # Destroys everything currently present in welcome screen
    for widget in window.winfo_children():
        widget.destroy()
